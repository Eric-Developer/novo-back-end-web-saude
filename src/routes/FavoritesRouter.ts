...
favoriteRouter.get(
  ...
  async (req: UserRequest, res: Response) => {
    try {
      const favorites = await favoritesService.getFavoritesByUser(
        Number(req.userId)
      );
        ...
        res.status(500).json({
          error:
            error instanceof Error
              ? error.message
              : 'Erro interno ao listar favoritos',
...
favoriteRouter.get(
  '/favorite/:healthUnitId',
  verifyToken(),
  async (req: UserRequest, res: Response) => {
    try {
      const favorite = await favoritesService.getFavoriteByHealthUnit(
        Number(req.userId),
        Number(req.params.healthUnitId)
      );
      ...
      if (error instanceof CustomError) {
        ...
        res.status(500).json({
          error:
            error instanceof Error
              ? error.message
              : 'Erro interno ao buscar favorito',
          stack: error instanceof Error ? error.stack : null,
        });
      }
    }
