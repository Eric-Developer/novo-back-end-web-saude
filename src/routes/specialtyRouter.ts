...
specialtyRouter.put("/specialty/:id", async (req: Request, res: Response) => {
    ...
    try {
        ...
        const updatedSpecialty = await specialtyService.updateSpecialty(Number(id), { name });
...
specialtyRouter.delete("/specialty/:id", async (req: Request, res: Response) => {
    const { id } = req.params;
...
specialtyRouter.get("/specialty/search", async (req: Request, res: Response) => {
    ...
    try {
        const specialties = await specialtyService.searchSpecialtiesByName(searchTerm);
        res.status(200).json(specialties);
    } catch (error) {
