...
class AuthService {
    ...
    try {
      ...
      if (existingPendingUser) {
        if (existingPendingUser.status === 'pending') {
          ...
          if (!secretKey) {
            throw new CustomError(
              'SECRET_KEY não está definido nas variáveis de ambiente',
              500,
              'MISSING_SECRET_KEY'
            );
          }

          const token = jwt.sign({ id: existingPendingUser.id }, secretKey, {
            expiresIn: '1h',
          });
      ...
      const token = jwt.sign({ id: pendingUser.id }, secretKey, {
        expiresIn: '1h',
      });
      sendAccountVerificationEmail(email, token);
  ...
  public async verifyEmailToken(
    ...
    const pendingUser = await this.pendingUserRepository.findOne({
      where: { email, status: PendingUserStatus.PENDING },
    });
    if (!pendingUser) {
      throw new CustomError(
        'Usuário não encontrado ou já verificado',
        404,
        'USER_NOT_FOUND'
      );
    }
    ...
    try {
      ...
      if (decodedToken.id !== pendingUser.id) {
        throw new CustomError(
          'Token inválido para o usuário fornecido.',
          400,
          'INVALID_TOKEN'
        );
      }
  ...
  public async signIn(email: string, password: string) {
    ...
    const token = jwt.sign({ id: user.id, type: user.user_type }, secretKey, {
      expiresIn: '12h',
    });
    return token;
  }
