...
class OperatingHoursService {
  ...
  public async getByHealthUnitId(
    healthUnitId: number
  ...
  private async validateOperatingHoursConflict(
    ...
    if (!data.health_unit_id || !data.day_of_week) {
      throw new CustomError(
        'Unidade de saúde e dia da semana são obrigatórios',
        400,
    ...
    const existingHours = await this.findExistingOperatingHours(
      data.health_unit_id,
      data.day_of_week
    );
  ...
  private async findExistingOperatingHours(
    healthUnitId: number,
    dayOfWeek: Day
  ) {
    return await this.operatingHoursRepository.find({
  ...
  private getDayName(day: Day): string {
    return (
      Object.keys(Day).find((key) => Day[key as keyof typeof Day] === day) ||
      day
    );
  }
}
