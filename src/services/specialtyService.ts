...
class SpecialtyService {
    ...
    public async createSpecialty(specialtyData: Partial<Specialty>) {
        if (!specialtyData.name) {
            throw new CustomError("O nome da especialidade é obrigatório.", 400, "SPECIALTY_NAME_REQUIRED");
        }
    ...
    public async updateSpecialty(id: number, updatedSpecialtyData: Partial<Specialty>) {
        const specialty = await this.specialtyRepository.findOne({ where: { id } });
        if (!specialty) {
            throw new CustomError("Especialidade não encontrada", 404, "SPECIALTY_NOT_FOUND");
        }

        // Se o nome da especialidade for alterado, verificar se o novo nome já existe
        if (updatedSpecialtyData.name && updatedSpecialtyData.name !== specialty.name) {
            const existingSpecialty = await this.specialtyRepository.findOne({
                where: {
    ...
    public async deleteSpecialty(id: number) {
        const specialty = await this.specialtyRepository.findOne({ where: { id } });
        if (!specialty) throw new CustomError("Especialidade não encontrada", 404, "SPECIALTY_NOT_FOUND");
